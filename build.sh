...
do_choose_pkgs() {
  RECIPES+=(zlib llvm libffi xorg-macros x11proto xorgproto xcbproto wayland wayland-protocols
            xdmcp xau xcb xcb-ewmh xtrans x11 xext vulkan-headers vulkan-loader spirv-headers
            spirv-tools robin-hood-hashing vulkan-validation-layers vulkan-tools libdrm xshmfence)
