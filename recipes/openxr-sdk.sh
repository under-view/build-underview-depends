...
do_compile_openxr-sdk() {
	cmake --build "${PACKAGES_DIR}/openxr-sdk/build" --config "${CMAKE_BUILD_TYPE}" -j $BUILDTHREADS || return $FAILURE
	return $SUCCESS
}


do_install_openxr-sdk() {
	cmake --build "${PACKAGES_DIR}/openxr-sdk/build" --config "${CMAKE_BUILD_TYPE}" --target install || return $FAILURE
	return $SUCCESS
}
